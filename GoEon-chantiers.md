...
*Mis à jour le 5 septembre 2026. Ce fichier bouge souvent ; le manuel de fabrication (`GoEon-conventions.md`) presque jamais.*
...
- ~~**Bascule automatique par date.**~~ **FAIT le 30/08** — `script.js` §18. Module générique : il balaie `.home-card[data-debut][data-fin]` sur l'accueil et pose « En cours ! » ou « Terminé » selon `Date.now()`, relu toutes les minutes. Les cinq cartes d'évènement sont datées, plus aucun badge n'est écrit en dur, et `.badge-termine` a rejoint `index.css` à côté de `.badge-en-cours`. Une date illisible ou inversée laisse la carte intacte, pour ne jamais afficher un faux badge. Testé sur huit instants simulés, dont les bascules à la minute près. *Ce chantier est né de trois retards constatés en août : Braises Arctiques, Goupilou et le badge « En cours » resté sur Festival Aquatique quatre jours après sa fin.*
  **Ce qui reste manuel** : le retrait de la carte à la fin d'un évènement, et la mise à jour de `data-fin` si Niantic prolonge. **Reste à faire** : brancher le même mécanisme sur les notifications. `evenements.json` existe depuis le 05/09 et son format de dates est déjà le bon.
...
- **Calendrier : alimenter `evenements.json` au-delà du 6 octobre.** Les données s'arrêtent à la rotation Xerneas / Méga-Empiflor et au Harvest Festival, tous saisis côté septembre. Passé le 6, l'aperçu de l'accueil se videra par le bas **sans le signaler** — c'est le seul entretien récurrent que la page demande, et il est invisible tant qu'on ne le cherche pas. Le mois `2026-10` existe déjà, vide, pour rendre octobre navigable.
...
- ~~**`robots.txt` et `sitemap.xml`**~~ **FAIT** — les deux fichiers sont au dépôt. `robots.txt` ouvre tout le site sauf `navbar.html` (fragment injecté dans `#nav-placeholder`, indexé seul il donnerait un résultat vide) et `regionaux.html` (page inachevée), et déclare le sitemap. Ses commentaires sont volontairement sans accents : le fichier est servi en `text/plain` sans charset, l'UTF-8 s'y affiche de travers. Le conflit redouté avec le `robots.txt` managé de Cloudflare est écarté — « Disable robots.txt configuration » est actif, le fichier du dépôt est servi seul.
  **Entretien** : deux lignes `Disallow` à retirer le jour où `regionaux.html` est reprise. Et `sitemap.xml` est à tenir à jour à chaque page ajoutée ou retirée — `Calendrier` y a été inscrit le 05/09, à la validation de la page.
